...
Investigate logging at the system, application, traditional Linux, and Nginx levels. The purpose of this section is to
...
### View logs for the Task Manager systemd service

```bash
sudo journalctl -u task-manager.service --no-pager -n 50
```

- `journalctl` — queries the systemd journal.
- `-u task-manager.service` — limits the output to the Task Manager systemd service.
- `--no-pager` — prints the output directly to the terminal instead of opening a pager.
- `-n 50` — displays the most recent 50 log entries.

The output confirmed that systemd records the lifecycle of the Task Manager service, including:
...
- Successful completion of the systemd service

For example, the journal showed:

```
systemd[1]: Starting task-manager.service - Task Manager Application...
docker[53355]: Container task-manager-api-1 Started
systemd[1]: Finished task-manager.service - Task Manager Application.
```

This provides a record of the systemd-level management of the application.
...
This command reports how much disk space the systemd journal is currently using.
...
Therefore, at the time of checking, the systemd journal was using approximately 32 MB of disk space.
...
The application writes its Uvicorn logs to the container's standard output/error streams. Docker collects these logs, making them accessible through Docker Compose.
...
This displays the logs generated by the services defined in the Compose file.

The application produced entries such as:
...
Application requests were also recorded, for example:

```
api-1 | INFO: 172.18.0.1:36568 - "GET /docs HTTP/1.0" 200 OK
api-1 | INFO: 172.18.0.1:41048 - "POST /tasks HTTP/1.0" 200 OK
api-1 | INFO: 172.18.0.1:44088 - "DELETE /tasks/1 HTTP/1.0" 404 Not Found
```

This demonstrates that application-level request activity is available through the Docker logging system.

### Check the container logging driver

```bash
sudo docker inspect task-manager-api-1 --format '{{json .HostConfig.LogConfig}}'
```
...
Therefore, the Task Manager container is currently using Docker's `json-file` logging driver.

No custom Docker logging configuration was identified from this inspection.
...
The directory was inspected with:

```bash
sudo ls -lah /var/log
```

Several important logs were present.

### `/var/log/syslog`

This contains general system activity and messages from various services.
...
The output contained messages from services such as systemd, sysstat, and the Amazon SSM Agent.

### `/var/log/auth.log`

This contains authentication and authorization-related events.
...
The output included sudo activity, showing commands executed through elevated privileges. For example, the log recorded ubuntu opening and closing sudo sessions and the commands executed as root.

This makes auth.log useful when investigating authentication, privilege escalation, or sudo-related activity.

### `/var/log/kern.log`

This contains messages generated by the Linux kernel.

### `/var/log/ufw.log`

This contains UFW firewall events.
...
These entries show that UFW was actively blocking incoming connection attempts.

### Other observed log locations

The `/var/log` directory also contained logs/directories associated with:
...
The exact purpose of each log depends on the corresponding service or package.

### Log rotation

The `/var/log` listing also showed rotated logs such as:
...
and compressed historical logs such as:
...
The active access log can be viewed with:

```bash
sudo tail -n 20 /var/log/nginx/access.log
```

The access log records incoming HTTP requests handled by Nginx.

For example, successful requests to the FastAPI application were recorded:

```
"GET /docs HTTP/1.1" 200
"GET /openapi.json HTTP/1.1" 200
"POST /tasks HTTP/1.1" 200
```

It also recorded unsuccessful requests, such as:

```
"GET / HTTP/1.1" 404
"DELETE /tasks/1 HTTP/1.1" 404
```

The access log therefore provides useful information about:
...
The logs also contained requests from automated internet scanners. Because the server is publicly reachable, unsolicited scanning traffic is expected and is recorded by Nginx.

### Nginx error log

The Nginx error log is: `/var/log/nginx/error.log`
...
No output was returned, meaning the error log was empty at the time it was checked.

This should not be interpreted as proof that Nginx has never experienced an error; it only records the state observed during this check.

### Nginx log rotation

The presence of files such as:
...
| Purpose | Command |
|---|---|
| Task Manager systemd logs | `sudo journalctl -u task-manager.service --no-pager -n 50` |
| Journal disk usage | `sudo journalctl --disk-usage` |
| Application logs | `sudo docker compose -f /opt/task-manager/docker-compose.yml logs` |
| Inspect Docker logging driver | `sudo docker inspect task-manager-api-1 --format '{{json .HostConfig.LogConfig}}'` |
| General system logs | `sudo tail -n 20 /var/log/syslog` |
| Authentication/sudo logs | `sudo tail -n 20 /var/log/auth.log` |
| UFW logs | `sudo tail -n 20 /var/log/ufw.log` |
| Nginx access logs | `sudo tail -n 20 /var/log/nginx/access.log` |
| Nginx error logs | `sudo tail -n 20 /var/log/nginx/error.log` |

**Conclusion:** Logging is currently implemented at the systemd, Docker/application, traditional Linux, UFW, and Nginx layers, without introducing an additional logging stack
